...
exports.register = async (req, res, next) => {
  try {
    ...
    if (!validatePassword(password)) return res.status(400).json({ success: false, message: 'Password min 6 chars' });
    ...
    res.status(201).json({ success: true, token: generateToken(user._id), user: { id: user._id, name, email, role: user.role } });
  } catch (err) { next(err); }
};
